...
app.post('/submit', function (req, res) {
    ...
    // Send the greeting in the format preferred by the WWW client
    switch (req.accepts(['html', 'text', 'json', 'xml'])) {
        case 'json':
            // Send the JSON greeting
            res.type('application/json');
            res.json({ welcome: `Witaj ${name}` });
            console.log(`The server sent a JSON document to the browser with name: ${name}`);
            break;

        case 'xml':
            // Send the XML greeting
            res.type('application/xml');
            res.send(`<welcome>Witaj ${name}</welcome>`);
            console.log(`The server sent an XML document to the browser with name: ${name}`);
        default:
            // Send the text plain greeting
            res.type('text/plain');
            res.send(`Witaj ${name}`);
            console.log(`The server sent a plain text to the browser with name: ${name}`);
        }
    });
